...
impl LyricsWindow {
    ...
    unsafe fn on_update(&mut self, _hwnd: HWND, _msg: u32, wparam: WPARAM, _lparam: LPARAM) -> LRESULT {
        // println!("!!!Receive",);
        // println!("!{:?}",_lparam.0);

        let param_ptr=  _lparam.0 as *const c_char;
        // println!("!{:?}",param_ptr);
        let param =  CStr::from_ptr(param_ptr).to_str().unwrap();
        println!("！！！！{:?}",param);
        let params:Vec<String> =param.split("&##&").map(|s| s.to_string()).collect();
        println!("！！！！{:?}",params);

            self.update_lines(params.get(0),params.get(1));
    ...
    fn update_lines(&mut self,line_current_new:Option<&String>,line_next_new:Option<&String>,) -> Result<()> {
        if self.line_current.as_ref() != line_current_new {
            self.line_current = line_current_new.cloned();
        }
        // if self.line_next.as_ref() != line_next_new {
        //     self.line_next = line_next_new.cloned();
        // }
        let line_next = match line_next_new {
            Some(line_next_new) if !line_next_new.is_empty() => Some(line_next_new),
            Some(line_next_new) if line_next_new.is_empty() => None,
        ...
        //         };
                if self.line_next.as_ref() != line_next {
                    self.line_next = line_next_new.map(|s| s.clone());
                    if !self
                        .line_next
                        .as_ref()
                        .map(|s| s.is_empty())
    ...
    fn get_or_init_resources(&self) -> Result<&Resources> {
        self.resources.get_or_try_init(|| {
            ...
            let _dcomp_device: IDCompositionDevice =  unsafe { DCompositionCreateDeviceW(&dxgi_device, ) }?;
...
pub unsafe fn DCompositionCreateDeviceW<'a, T: ::windows::Interface>(
    dxgidevice: impl ::windows::IntoParam<'a, IDXGIDevice>,
) -> ::windows::Result<T> {
    #[cfg(windows)]
        {
            #[link(name = "dcomp")]
            ...
            DCompositionCreateDevice(
                ...
                &mut result__  as *mut _ as *mut *mut _,
            )
                .and_some(result__)
        }
    #[cfg(not(windows))]
